...
def export_for_sam2(
    ...
    ----------
    exclude_person_ids:
        UI에서 사용자가 선택한 블러 제외 person_id 집합.
        값이 들어오면 Top-N 기준보다 이 선택값을 우선 사용한다.

    blur_main_person:
        exclude_person_ids가 None일 때만 사용된다.
    ...
    if exclude_person_ids is not None:
        blur_person_ids = set(person_db.keys()) - set(exclude_person_ids)
    else:
    ...
    logger.info(
        "SAM2 input export 완료 | blur 대상 track 수=%d | 제외 person 수=%d | 저장=%s",
        len(sam2_inputs),
        len(exclude_person_ids) if exclude_person_ids is not None else 0,
        output_path,
    )
